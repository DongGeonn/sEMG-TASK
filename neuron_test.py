...
print("36*25*17")
model=nn.Sequential(nn.Linear(36,25),nn.ReLU(), nn.Linear(25,17))                # 다층 퍼셉트론으로 분류
optimizer=optim.SGD(model.parameters(), lr=1e-1)
losses_36_25_17=[]
nb_epochs=10000
for epoch in range(nb_epochs+1):
    ...
    cost.backward()
    optimizer.step()
    losses_36_25_17.append(cost.item())
    if epoch%1000==0:
        print('Epoch {}/{} Cost: {:.6f}'.format(epoch, nb_epochs, cost.item()))
...
print("36*75*17")    
model=nn.Sequential(nn.Linear(36,75),nn.ReLU(), nn.Linear(75,17))                # 다층 퍼셉트론으로 분류
optimizer=optim.SGD(model.parameters(), lr=1e-1)
losses_36_75_17=[]
nb_epochs=10000
for epoch in range(nb_epochs+1):
    ...
    cost.backward()
    optimizer.step()
    losses_36_75_17.append(cost.item())
    if epoch%1000==0:
        print('Epoch {}/{} Cost: {:.6f}'.format(epoch, nb_epochs, cost.item()))
...
print("36*100*17")        
model=nn.Sequential(nn.Linear(36,100),nn.ReLU(), nn.Linear(100,17))              
optimizer=optim.SGD(model.parameters(), lr=1e-1)
losses_36_100_17=[]
nb_epochs=10000
for epoch in range(nb_epochs+1):
    ...
    cost.backward()
    optimizer.step()
    losses_36_100_17.append(cost.item())
    if epoch%1000==0:
        print('Epoch {}/{} Cost: {:.6f}'.format(epoch, nb_epochs, cost.item()))
